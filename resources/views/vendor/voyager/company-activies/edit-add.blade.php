...
@section('content')
    <div class="page-content container-fluid">
        <form class="form-edit-add" role="form" action="@if(isset($dataTypeContent->id)){{ route('voyager.company-activies.update', $dataTypeContent->id) }}@else{{ route('voyager.company-activies.store') }}@endif" method="POST" enctype="multipart/form-data">
                ...
                <div class="col-md-8">
                    ...
                    <div class="panel">
                        ...
                        <div class="panel-body">
                            @php
                                $dataTypeRows = $dataType->{(isset($dataTypeContent->id) ? 'editRows' : 'addRows' )};
                                $exclude = ['title', 'body', 'excerpt', 'slug', 'status','company_category_id', 'category_id', 'author_id', 'featured', 'image', 'meta_description', 'meta_keywords', 'seo_title'];
                            @endphp
                ...
                <div class="col-md-4">
                    ...
                    <div class="panel panel panel-bordered panel-warning">
                        ...
                        <div class="panel-body">
                            ...
                            <div class="form-group">
                                <label for="company_category_id">{{ __('voyager::post.category') }}</label>
                                <select class="form-control" name="company_category_id">
                                    @foreach(App\Models\CompanyCategory::all() as $category)
                                        <option value="{{ $category->id }}"@if(isset($dataTypeContent->company_category_id) && $dataTypeContent->company_category_id == $category->id) selected="selected"@endif>{{ $category->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
