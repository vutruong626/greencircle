...
<div class="page-content container-fluid">
    <form class="form-edit-add" role="form"
        action="@if(isset($dataTypeContent->id)){{ route('voyager.news.update', $dataTypeContent->id) }}@else{{ route('voyager.news.store') }}@endif"
        ...
        <div class="row">
            <div class="col-md-8">
                <div class="panel">
                    ...
                    <div class="panel">
                    <div class="panel-heading">
                        <h3 class="panel-title">Mô tả ngắn cho tin tức</h3>
                        <div class="panel-actions">
                ...
                <div class="panel">
                    <div class="panel-heading">
                        <h3 class="panel-title">Nội dung cho tin tức</h3>
                        <div class="panel-actions">
                            <a class="panel-action voyager-resize-full" data-toggle="panel-fullscreen" aria-hidden="true"></a>
                        </div>
                    </div>
                ...
                <div class="panel-body">
                    <input type="number" class="form-control" id="numerical_order" name="numerical_order" placeholder="Vị chí thứ 1"
                        value="@if(isset($dataTypeContent->numerical_order)){{ $dataTypeContent->numerical_order }}@endif">
                </div>
            </div>
            <div class="col-md-4">
                <!-- ### DETAILS ### -->
                <div class="panel panel panel-bordered panel-warning">
                    <div class="panel-heading">
                        <h3 class="panel-title"><i class="icon wb-clipboard"></i> Chi tiết</h3>
                        <div class="panel-actions">
                            <a class="panel-action voyager-angle-down" data-toggle="panel-collapse"
                                aria-hidden="true"></a>
                        </div>
                    ...
                    <div class="panel-body">
                        ...
                        <div class="form-group">
                            <label for="category_news_id">Danh mục bài viêt</label>
                            <select class="form-control" name="category_news_id">
                                @foreach(App\Models\CategoryNews::all() as $category_news)
                                <option value="{{ $category_news->id }}" @if(isset($dataTypeContent->category_news_id)
                                    &&
                                    $dataTypeContent->category_news_id == $category_news->id)
                                    selected="selected"@endif>{{ $category_news->name }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
